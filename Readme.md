...
- [ ] **Workflow "aanbiedingen ophalen"**: vul in de node *Instellingen aanbiedingen* het chat-id van jullie groep in (staat bewust niet in de repo), selecteer de Telegram- en Gemini-credentials in de betreffende nodes, en kies in *Boodschappenhistorie ophalen* de tabel `items` en in *Maaltijdhistorie ophalen* de tabel `maaltijden` — die twee bepalen samen welke aanbiedingen als "van jullie" gelden.
...
| De node *Match & weekbericht* geeft niets terug, of het weekbericht blijft uit | Meestal is het chat-id leeg in de node *Instellingen aanbiedingen* — dat veld staat bewust niet in de repo, dus na een import moet je het opnieuw invullen (hetzelfde id als in `allowedChatIds` van de hoofdworkflow). De node geeft hier sinds kort een duidelijke foutmelding over in plaats van een lege output |
| Het weekbericht komt twee keer (of de nodes na een Merge draaien dubbel) | Kijk of er twee lijnen op dezelfde **ingang** van de Merge-node uitkomen. Een Merge draait namelijk opnieuw zodra een ingang gevuld wordt, dus alles erachter gebeurt dan dubbel. Verwijder de lijn die er niet hoort (of geef de Merge een extra ingang). `python3 scripts/valideer-workflows.py` spoort dit op |
