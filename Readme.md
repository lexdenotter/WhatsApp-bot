...
| Een tool vindt niets, terwijl er wel rijen in de tabel staan | Open de lees-node en zet **Must Match** op **All Conditions**. Staat die op *Any Condition* (de standaard) én zijn er geen voorwaarden, dan matcht "één van nul voorwaarden" niets en krijg je stil nul rijen. `python3 scripts/valideer-workflows.py` spoort dit op |
